...
Привет, ${name}! Вы пока не готовы повышать уровень,вам нужно набрать *${max - user.exp}* опыта для повышения уровня и достижения новых высот! Опыт можно повысить использовав команды из эконом меню - работать(.work),ходить на преключения(.adventure),добывать ресурсы(.mine),или воровать у других игроков(.rob),а также играя в игры бота,также опыт немного повышается за любое сообщение в группе с ботом, ну или купить за реальные деньги(.donate). Продолжайте в том же духе, и скоро боты будут петь вам дифирамбы! 🚀
`.trim();
